...
public class BackButton: NSObject {
    
    public class func createWithText(_ text: String, color: UIColor, target: AnyObject?, action: Selector) -> [UIBarButtonItem] {
        let backArrowImage = imageOfBackArrow(color: color).withAlignmentRectInsets(UIEdgeInsets(top: 0, left: -8.0, bottom: 0, right: 0))
        ...
        backTextButton.setTitlePositionAdjustment(UIOffset(horizontal: -20.0, vertical: 0.0), for: UIBarMetrics.default)
        return [backArrowButton, backTextButton]
    }
    
    public class func createWithImage(_ image: UIImage, color: UIColor, target: AnyObject?, action: Selector) -> [UIBarButtonItem] {
    ...
    private class func imageOfBackArrow(_ size: CGSize = CGSize(width: 14, height: 18), color: UIColor = UIColor(hue: 0.59, saturation: 0.674, brightness: 0.886, alpha: 1), resizing: ResizingBehavior = .AspectFit) -> UIImage {
        var image: UIImage
